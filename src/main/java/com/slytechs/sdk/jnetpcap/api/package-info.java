 ...
 *
 * <p>
 * This package provides {@link NetPcap}, a high-level packet capture interface
 * that wraps the low-level {@link com.slytechs.sdk.jnetpcap.Pcap} bindings and
 * integrates with protocol dissection. Packets delivered through
 * {@code dispatch()}, {@code loop()}, {@code next()}, and {@code nextEx()} are
 * fully dissected with protocol headers accessible via the zero-allocation
 * {@code hasHeader()} pattern.
 * </p>
 *
 * <h2>API Layers</h2>
 *
 * <p>jNetPcap provides two API layers:</p>
 *
 * <table>
 * <caption>jNetPcap API Layers</caption>
 * <tr><th>Class</th><th>Module</th><th>Description</th></tr>
 * <tr><td>{@link com.slytechs.sdk.jnetpcap.Pcap}</td><td>jnetpcap-bindings</td>
 *     <td>Low-level libpcap bindings — direct 1:1 native access</td></tr>
 * <tr><td>{@link NetPcap}</td><td>jnetpcap-api</td>
 *     <td>High-level API with integrated protocol dissection</td></tr>
 * </table>
 *
 * <h2>Configuration: NetPcap vs PacketSettings</h2>
 *
 * <p>There are two distinct configuration concerns:</p>
 *
 * <table>
 ...
 * <tr><td>{@link com.slytechs.sdk.protocol.core.PacketSettings}</td>
 *     <td>Packet structure &amp; memory</td>
 *     <td>{@code dissect()}, {@code zeroCopy()}, {@code descriptorType()},
 *         etc.</td></tr>
 * </table>
 *
 * <h3>NetPcap Configuration</h3>
 * <p>
 * NetPcap setters configure traditional pcap capture properties — how packets
 * are captured from the network or read from files:
 * </p>
 * <ul>
 ...
 * <p>
 * {@link com.slytechs.sdk.protocol.core.PacketSettings} configures how packets
 ...
 * <tr><td>{@link com.slytechs.sdk.protocol.core.PacketSettings}</td>
 *     <td>Packet structure and memory configuration</td></tr>
 * <tr><td>{@link PacketHandler}</td><td>Packet callback interfaces</td></tr>
 * <tr><td>{@link com.slytechs.sdk.jnetpcap.Pcap}</td>
 *     <td>Low-level libpcap bindings</td></tr>
 * <tr><td>{@link com.slytechs.sdk.jnetpcap.PcapIf}</td>
 *     <td>Network interface descriptor</td></tr>
 * <tr><td>{@link com.slytechs.sdk.jnetpcap.BpFilter}</td>
 *     <td>Compiled BPF filter</td></tr>
 * </table>
 *
 * <h2>Memory Model</h2>
 *
 * <p>jNetPcap uses a layered memory model optimized for performance:</p>
 *
 * <pre>
 * ┌─────────────────────────────────────────────────────────────────┐
 ...
 * <tr><td>ScopedMemory</td><td>None (bind only)</td><td>Callback scope</td>
 *     <td>Zero-copy capture</td></tr>
 * <tr><td>FixedMemory</td><td>Pool or Arena</td><td>Until recycled</td>
 *     <td>Persistent packets</td></tr>
 * <tr><td>Hybrid</td><td>Mixed</td><td>Mixed</td>
 *     <td>Zero-copy data + fixed descriptor</td></tr>
 * </table>
 *
 ...
 * <tr><td>Eager</td><td>{@code dissect()}</td>
 *     <td>Full dissection before callback, TYPE2 descriptor</td></tr>
 * <tr><td>On-demand</td><td>{@code onDemand()}</td>
 *     <td>Dissect lazily on first {@code hasHeader()}</td></tr>
 * <tr><td>None</td><td>(default)</td>
 *     <td>No dissection, PCAP descriptor only</td></tr>
 * </table>
 *
 * <h3>Zero-Allocation Header Access</h3>
 *
 * <p>
 * The {@code hasHeader(Header)} pattern provides zero-allocation protocol
 ...
 *
 * <p>For tunneled packets (GRE, VXLAN, etc.), use the depth parameter:</p>
 *
 * <pre>{@code
 * Ip4 outerIp = new Ip4();
 * Ip4 innerIp = new Ip4();
 ...
 *
 * <p>
 * Packets in callbacks are bound to native buffers and valid only within
 * callback scope. To keep packets beyond the callback, use the persistence API:
 * </p>
 *
 * <table>
 * <caption>Persistence Methods</caption>
 * <tr><th>Method</th><th>Returns</th><th>Memory</th><th>Use Case</th></tr>
 * <tr><td>{@code persist()}</td><td>Same or copy</td><td>Fixed</td>
 *     <td>Keep beyond callback scope</td></tr>
 * <tr><td>{@code persistTo(pool)}</td><td>Pooled copy</td><td>Pool memory</td>
 *     <td>High-volume zero-GC capture</td></tr>
 * <tr><td>{@code copy()}</td><td>New copy</td><td>Auto-managed</td>
 *     <td>Independent lifecycle</td></tr>
 * <tr><td>{@code duplicate()}</td><td>New object</td><td>Shared (+ref)</td>
 *     <td>Parallel processing</td></tr>
 * <tr><td>{@code recycle()}</td><td>void</td><td>Returns to pool</td>
 *     <td>Release pooled packet when done</td></tr>
 * </table>
 *
 * <pre>{@code
 ...
 * <tr><td>{@code PCAP_PADDED}</td><td>24 bytes</td>
 *     <td>Kernel format (x64 padded)</td></tr>
 * <tr><td>{@code PCAP_PACKED}</td><td>16 bytes</td>
 *     <td>File format (packed)</td></tr>
 * <tr><td>{@code TYPE2}</td><td>96 bytes</td>
 *     <td>Full protocol dissection results</td></tr>
 * </table>
 *
 * <h2>Thread Safety</h2>
 *
 ...
 *
 * <ul>
 * <li>Capture on a dedicated thread</li>
 * <li>Use {@code persist()} for packets leaving the capture thread</li>
 * <li>Each worker thread needs its own header instances</li>
 * </ul>
 *
 * <pre>{@code
 ...
 * <tr><th>Method</th><th>Respects Timeout</th><th>Description</th></tr>
 * <tr><td>{@code loop(count, handler)}</td><td>No</td>
 *     <td>Process until count reached or {@code breakloop()} called</td></tr>
 * <tr><td>{@code dispatch(count, handler)}</td><td>Yes</td>
 *     <td>Process available packets up to count</td></tr>
 * <tr><td>{@code next()}</td><td>Yes</td>
 *     <td>Return next packet or null on timeout</td></tr>
 * <tr><td>{@code nextEx()}</td><td>Yes</td>
 *     <td>Return next packet with explicit status code</td></tr>
 * </table>
 *
 ...
 * <tr><td>Linux</td><td>libpcap 1.0+</td><td>Best performance</td></tr>
 * <tr><td>Windows</td><td>Npcap 1.0+</td>
 *     <td>Administrator rights required for live capture</td></tr>
 * <tr><td>macOS</td><td>libpcap (system)</td>
 *     <td>BPF device access required</td></tr>
 * </table>
 *
